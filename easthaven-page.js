...
async function loadData() {
    try {
        const response = await fetch('easthaven-restaurants.json');
...
// Town color for East Haven
const townColor = '#8B5CF6';
...
function createMarkerIcon(isFeatured) {
    ...
    return L.divIcon({
        ...
        "><span style="
            transform: rotate(45deg);
            color: ${isFeatured ? townColor : 'white'};
            font-size: ${fontSize};
        ">🍴</span></div>`,
        iconSize: [size, size],
