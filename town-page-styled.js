...
function sortByDistance(restaurants, userLat, userLng) {
    return restaurants
        .map(r => {
            if (r.lat && r.lng) {
                return { ...r, distance: calculateDistance(userLat, userLng, r.lat, r.lng) };
            }
            return r;
        })
        .sort((a, b) => {
...
// Render featured restaurants with distance
function renderFeaturedWithDistance() {
    const grid = document.getElementById('featuredGrid');
    ...
    const sorted = userLocation
        ? sortByDistance(data.featured, userLocation.lat, userLocation.lng)
        : data.featured;

    grid.innerHTML = sorted.map(restaurant => {
        const distanceBadge = restaurant.distance !== undefined
            ? `<span class="distance-badge">${formatDistance(restaurant.distance)}</span>`
            : '';
        return `
        <a href="${restaurant.website || '#'}" class="featured-card" target="_blank" rel="noopener noreferrer">
            <div class="card-image ${restaurant.darkBg ? 'dark-bg' : ''}">
            ...
            <div class="card-content">
                <h3>${restaurant.name}${distanceBadge}</h3>
                <span class="category-tag">${restaurant.category}</span>
                <div class="info-row">
                    <svg viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                    ${restaurant.address || townName}
    ...
    nearMeBtn.addEventListener('click', () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                // Re-render featured restaurants with distance
                renderFeaturedWithDistance();
